...
%% Parametreler
% Hareket uzayı
Space.size = [120, 4, 3];  % Uzay boyutları [m]
...
Distance = 10:20:110;     % Nesnenin hareket edeceği mesafe [m]

DropCounter = zeros(length(Distance), length(Object.velocity));
...
for k = 1:50
    for d = 1:length(Distance)
        for v = 1:length(Object.velocity)
            ...
            Time.tmax = Distance(d)/Object.velocity(v);      % Simülasyon süresi [sn]
            ...
            for i = 1:length(Time.axis)
                ...
                DropCounter(d, v) = DropCounter(d, v) + Counter;
...
for i = 1:length(Object.velocity)
    plot(Distance, DropCounter(:, i), "LineWidth", 1, "Marker", "+", ...
         "DisplayName", "Hareket Hızı " + num2str(Object.velocity(i)) + ...
         " [m/sn]");
end
xlabel("Yağmurda Koşma Mesafesi [m]", "FontSize", 12);
ylabel("Temas Eden Damla Sayısı", "FontSize", 12);
legend("Location", "northwest", "FontSize", 12);

exportgraphics(f1, "ConstantDistance_Results.jpeg", "Resolution", 600);
